...
### 🎨 権利方針: キャラ画像は**掲載停止中** (2026-08-10 改定・許諾申請中)

経緯:
1. 公式サポート (help@nikke-jp.com) に画像使用可否を照会 (2026-07-31 送信)
2. 一次返信「ガイドラインについて調査中」(2026-07-31) → **返信を待たず takedown 方式で掲載**する運営判断
3. **二次返信 (2026-08-10): 「詳細は二次創作ガイドラインを参照」= 個別許諾は出ない**という回答で確定
4. ガイドライン**第1条4項**「当社オリジナルコンテンツを**そのまま再現、複製、コピー、トレースする行為**、
   又はこれらと同様とみなされる行為は、**二次創作活動と認められません**」に照らし、
   ゲーム画像の掲載は許諾の対象外と判断 → **`USE_CHAR_IMAGES = false` に切り替え (掲載停止)**
5. 権利元 SHIFT UP CORP. の窓口 (`business@shiftup.co.kr` — ガイドライン第2条5項が指定) へ
   **利用許諾を申請中**。許諾が得られた場合のみ再開する

> ⚠ **`USE_CHAR_IMAGES` を独断で true に戻さないこと。** 許諾の確認が前提です。
...
- 表示は `js/tiles.js` の**ハイブリッド**(現在は自作タイル側のみ稼働): 画像があるキャラは
